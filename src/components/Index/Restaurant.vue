<template>
  <div class="restaurants">
    <div v-for="value in restaurants" :key="value.id" @click="next(value.id)">
      <Row>
        <div class="ivu-col-img">
          <img :src="value.image_path" />
        </div>
        <div class="ivu-col-text">
          <p class="Rname">
          ...
          <p>
            ...
            <span class="delivery" v-show="value.delivery_mode">{{
              value.delivery_mode ? value.delivery_mode.text : ""
            }}</span>
          </p>
          <p>
            <span>
          ...
          <div
            class="activity"
            :class="{ activityShow: value.isShow }"
            v-show="value.activity"
          >
            <p v-for="v in value.activity" :key="v.id">
              <span class="activity1" v-if="v.icon_name == '首'">{{
                v.icon_name
              }}</span>
              <span
                class="activity2"
                v-else-if="v.icon_name == '减' || v.icon_name == '折'"
                >{{ v.icon_name }}</span
              >
              <span
                class="activity3"
                v-else-if="v.icon_name == '换' || v.icon_name == '特'"
                >{{ v.icon_name }}</span
              >
              <span class="activity4" v-else-if="v.icon_name == '新'">{{
                v.icon_name
              }}</span>
            ...
            <div
              @click.stop="toggleDetail(index, value.id)"
              v-show="value.activity.length + value.supports.length > '2'"
            >
              {{ value.activity.length + value.supports.length }}个活动
              <Icon
                type="md-arrow-dropdown"
                :class="{ dropUp: value.isShow }"
              />
            </div>
          </div>
        </div>
